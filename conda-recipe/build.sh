...
## - WITH_CPLEX
##    - Build cnnef with CPLEX enabled
##
...
if [[ "$WITH_CPLEX" == "" ]]; then
    ...
    if [ "$CPLEX_ROOT_DIR" == "<UNDEFINED>" ]; then
        ...
        echo "* environment before building cnnef.     *"
        echo "******************************************"
        exit 1
    fi
...
##
## Install to prefix
cp -r ${SRC_DIR}/build/python/cnnef ${PREFIX}/lib/python2.7/site-packages/


CNNEF_MODULE_SO=${PREFIX}/lib/python2.7/site-packages/cnnef/_cnnef.so
...
if [[ "$WITH_CPLEX" != "" ]]; then
    (
        if [ `uname` == "Darwin" ]; then
            ...
            install_name_tool -change ${CPLEX_LIB_DIR}/libcplex.dylib     @rpath/libcplex.dylib    ${CNNEF_MODULE_SO}
            install_name_tool -change ${CPLEX_LIB_DIR}/libilocplex.dylib  @rpath/libilocplex.dylib ${CNNEF_MODULE_SO}
            install_name_tool -change ${CONCERT_LIB_DIR}/libconcert.dylib @rpath/libconcert.dylib  ${CNNEF_MODULE_SO}
        fi

        # Rename the cnnef package to 'cnnef_with_cplex'
        cd "${PREFIX}/lib/python2.7/site-packages/"
        mv cnnef cnnef_with_cplex
    )
fi
...
if [[ "$WITH_GUROBI" != "" ]]; then
    (
        if [ `uname` == "Darwin" ]; then
            ...
            install_name_tool -change $fullpath @rpath/$(basename $fullpath) ${CNNEF_MODULE_SO} 
        fi

        # Rename the cnnef package to 'cnnef_with_gurobi'
        cd "${PREFIX}/lib/python2.7/site-packages/"
        mv cnnef cnnef_with_gurobi
    )
fi
